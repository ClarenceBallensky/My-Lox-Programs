...
static void switchStatement() {
  ...
  int endJump = -1; //jump to the end after a matched case

  while (!check(TOKEN_RIGHT_BRACE) && !check(TOKEN_DEFAULT)) {
    consume(TOKEN_CASE, "Expect 'case'.");
    ...
    // --- NOT MATCHED ---
    patchJump(caseJump);
    emitByte(OP_POP); // pop comparison result if false
    // --- end NOT MATCHED ---
  }
  ...
  //patch final jump to end
  if (endJump != -1) {
    patchJump(endJump);
  }

  //pop the original switch value
  emitByte(OP_POP);
