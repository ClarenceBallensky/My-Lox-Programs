...
//> interpret-methods
    ...
    Map<String, LoxFunction> staticMethods = new HashMap<>(); //CH12, Q1
...
//< interpreter-method-initializer
    ...
    //create metaclass
    LoxClass metaclass = new LoxClass(null, stmt.name.lexeme + " metaclass", null, staticMethods);

    //add static methods to the metaclass fields
    /*for (Map.Entry<String, LoxFunction> entry : staticMethods.entrySet()) {
      Token token = new Token(TokenType.IDENTIFIER, entry.getKey(), null, 0);
      metaclass.set(token, entry.getValue());
    } */
...
//> Inheritance interpreter-construct-class
    LoxClass klass = new LoxClass((LoxClass)metaclass, stmt.name.lexeme,
        (LoxClass)superclass, instanceMethods);
//> end-superclass-environment

    environment.define(stmt.name.lexeme, klass); //CH12, Q1
...
*/
    environment.assign(stmt.name, klass); //CH12, Q1
    return null;
  }
//< Classes interpreter-visit-class
