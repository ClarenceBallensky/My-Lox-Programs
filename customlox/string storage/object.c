...
//alternative to constantString -- strings own memory instead of borrowing it
...
//new alternative to copyString -- these strings borrow memory instead of owning it
ObjString* constantString(const char* start, int length) {
  
  //uint32_t hash = hashString(chars, length);
  //return allocateString((char*)chars, length, hash, false);
  
  if (length < 0) length = 0;  // Prevent negative lengths
    char* chars = ALLOCATE(char, length + 1);
    memcpy(chars, start, length);
    chars[length] = '\0';
    return allocateString(chars, length, hashString(chars, length), true);
}
...
//< Calls and Functions print-native
    case OBJ_STRING:
      ...
      printf("%s\n", s->chars);  // Only the string characters
      break;
