...
//MY CODE START
static uint32_t readConstantLong(CallFrame* frame) {
  uint32_t b1 = (uint8_t)*frame->ip++;
  uint32_t b2 = (uint8_t)*frame->ip++;
  uint32_t b3 = (uint8_t)*frame->ip++;
  return (b1 << 16) | (b2 << 8) | b3;
}

#define READ_CONSTANT_LONG() \
  (frame->closure->function->chunk.constants.values[readConstantLong(frame)])
//MY CODE END
