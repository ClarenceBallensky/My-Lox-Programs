...
//< existing-in-scope
  ...
  printf("setting isFinal for %.*s = %d\n",
       name->length, name->start,
       current->declaringFinal); 

  printf("declaringFinal = %d\n", current->declaringFinal);

  current->locals[current->localCount - 1].isFinal = current->declaringFinal;
...
//> named-variable-can-assign
  if (canAssign && match(TOKEN_EQUAL)) {
    ...
    if (local != -1 && current->locals[local].isFinal) {
...
static void varDeclaration() {
  current->declaringFinal = match(TOKEN_FINAL);
...
//> for-initializer
  if (match(TOKEN_SEMICOLON)) {
    ...
    current->declaringFinal = true;
    consume(TOKEN_VAR, "Expect 'var' after 'final");
    varDeclaration();
    current->declaringFinal = false;
